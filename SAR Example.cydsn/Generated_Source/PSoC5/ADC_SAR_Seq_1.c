...
static uint8 ADC_SAR_Seq_1_tempTD = CY_DMA_INVALID_TD;
static uint8 ADC_SAR_Seq_1_finalTD = CY_DMA_INVALID_TD;
...
void ADC_SAR_Seq_1_Init(void)
{
    ...
    ADC_SAR_Seq_1_finalChan = ADC_SAR_Seq_1_FinalBuf_DmaInitialize((uint8)ADC_SAR_Seq_1_FINAL_BYTES_PER_BURST,
...
void ADC_SAR_Seq_1_Enable(void)
{
    ...
    (void)CyDmaClearPendingDrq(ADC_SAR_Seq_1_tempChan);
    (void)CyDmaClearPendingDrq(ADC_SAR_Seq_1_finalChan);
    ...
    */
    ...
    if (ADC_SAR_Seq_1_tempTD == DMA_INVALID_TD)
    {
    ...
    */
    ...
    if (ADC_SAR_Seq_1_finalTD == DMA_INVALID_TD)
    {
...
void ADC_SAR_Seq_1_Stop(void)
{
    ...
    *
    * Side Effects:
    *  Calling ADC_SAR_Seq_1_StartConvert() disables the external SOC pin.
    *
    * Reentrant:
    *  No.
    *
...
*   Values:
*         - ADC_SAR_Seq_1_RETURN_STATUS      - Immediately returns the 
*                                                 status
*         - ADC_SAR_Seq_1_WAIT_FOR_RESULT    - Does not return a result 
...
*
* Side Effects:
*  Affects ADC_SAR_Seq_1_CountsTo_Volts(), 
*  ADC_SAR_Seq_1_CountsTo_mVolts(), and ADC_SAR_Seq_1_CountsTo_uVolts() 
...
*  ADC_SAR_Seq_1_StopConvert(), change the resolution, then restart the
*  conversions with ADC_SAR_Seq_1_StartConvert().
*  If you decide not to stop conversions before calling this API, you
*  should use ADC_SAR_Seq_1_IsEndConversion() to wait until conversion is 
...
*  ADC_SAR_Seq_1_SetResolution() is called before data is valid again.
*  Affects ADC_SAR_Seq_1_CountsTo_Volts(), ADC_SAR_Seq_1_CountsTo_mVolts(), 
*  and ADC_SAR_Seq_1_CountsTo_uVolts() by calculating the correct conversion 
...
* Side Effects:
*  Affects ADC_SAR_Seq_1_CountsTo_Volts(), ADC_SAR_Seq_1_CountsTo_mVolts(),
*  ADC_SAR_Seq_1_CountsTo_uVolts() by supplying the correct conversion 
