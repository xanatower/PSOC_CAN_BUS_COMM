...
void Timer_Int_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)Timer_Int__INTC_NUMBER] = address;
}
...
cyisraddress Timer_Int_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)Timer_Int__INTC_NUMBER];
}
...
{
    *Timer_Int_INTC_PRIOR = priority << 5;
}
...
uint8 Timer_Int_GetPriority(void)
{
    ...
    priority = *Timer_Int_INTC_PRIOR >> 5;
